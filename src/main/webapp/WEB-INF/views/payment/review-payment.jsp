<%-- Document : review-payment Created on : 8 Jun 2025, 3:03:07 am Author : Aiman --%>
        ...
        <div class="container-fluid px-5 my-4">
            ...
            <div class="row">
                ...
                <form class="col-auto my-3">
                    ...
                    <input class="form-control mb-2" type="text" name="bookingID"
                           value="${param.status != null ? param.bookingID : ''}"
                           placeholder="Search by booking ID">
                    <select name="status" class="form-select d-inline-block" aria-label="Default select"
                            style="width:200px;">
                        <option value="">--Filter by status--</option>
                        <option value="not_paid" ${param.status=='not_paid' ? 'selected' : '' }>Not Paid
                        </option>
                        <option value="paid" ${param.status=='paid' ? 'selected' : '' }>Paid</option>
                        <option value="refunded" ${param.status=='refunded' ? 'selected' : '' }>Refunded
                        </option>
                    </select>
                ...
                <table class="table table-dark ">
                    ...
                    <tbody class="table-hover">
                        ...
                        <c:forEach items="${bookings}" var="booking">
                            ...
                            <c:if
                                test="${(param.status == null || param.status == '' || booking.payment.status == param.status) && (param.bookingID == null || param.bookingID == '' || param.bookingID == booking.id)}">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <button type="button"
                                                        ...
                                                        data-bs-target="#payment-${booking.id}-Modal">
                                                    <c:out value="${booking.payment.status}" />
                                                </button>

                                                <form
                                                    action="${pageContext.request.contextPath}/updatePayment"
                                                    method="POST">
                                                    <!-- Modal -->
                                                    <div class="modal fade" id="payment-${booking.id}-Modal"
                                                         tabindex="-1"
                                                         aria-labelledby="payment-${booking.id}-ModalLabel"
                                                         aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered">
                                                            <div class="modal-content bg-dark text-white">
                                                                <div class="modal-header">
                                                                    <h1 class="modal-title fs-5"
                                                                        id="payment-${booking.id}-ModalLabel">
                                                                        Change Payment Status</h1>
                                                                ...
                                                                <div
                                                                    ...
                                                                    <input type="radio" class="btn-check "
                                                                           name="paymentStatus"
                                                                           id="not_paid-${booking.id}"
                                                                           value="not_paid" autocomplete="off"
                                                                           ${booking.payment.status=='not_paid'
                                                                             ? 'disabled' : '' }>
                                                                    <label
                                                                        class="btn btn-secondary status-item"
                                                                        for="not_paid-${booking.id}">Not
                                                                        Paid</label>
                                                                    <input type="radio" class="btn-check"
                                                                           name="paymentStatus"
                                                                           id="paid-${booking.id}" value="paid"
                                                                           autocomplete="off"
                                                                           ${booking.payment.status=='paid'
                                                                             ? 'disabled' : '' }>
                                                                    <label
                                                                        class="btn btn-secondary status-item"
                                                                        for="paid-${booking.id}">Paid</label>
                                                                    <input type="radio" class="btn-check"
                                                                           name="paymentStatus"
                                                                           id="refunded-${booking.id}"
                                                                           value="refunded" autocomplete="off"
                                                                           ${booking.payment.status=='refunded'
                                                                             ? 'disabled' : '' }>
                                                                    <label
                                                                        class="btn btn-secondary status-item"
                                                                        for="refunded-${booking.id}">Refunded</label>
