            ...
            if (status != 0) {
                if (userSession.getRole().equals("member")) {
                    ...
                    response.sendRedirect(
                            request.getContextPath() + "/viewMember?message=Successfully deleted a member");
                    return;
                }
            } else {
        ...
        if (path.equals("/updateMember")) {
            ...
            if (UserDAO.isEmailUsed(email)) {
                if (!UserDAO.getUserById(id).getEmail().equals(email)) {
                    response.sendRedirect(request.getContextPath() + "/member?error=Email has been used");
                    return;
                }
            }
            if (UserDAO.isUsernameUsed(username)) {
                if (!UserDAO.getUserById(id).getUsername().equals(username)) {
                    response.sendRedirect(request.getContextPath() + "/member?error=Username has been used");
                    return;
                }
            }
            ...
            if (status != 0) {
                ...
                response.sendRedirect(
                        request.getContextPath() + "/member?message=Successfully updated " + member.getUsername());
            } else {
                response.sendError(501);
            }
        }
